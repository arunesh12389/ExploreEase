...
export function TrendingCard({ place, rank }) {
  return (
    // The <Link> component now wraps the <Card> directly.
    <Link href={`/place/${place.id}`}>
      {/* We've removed the extra <a> tag and moved its data-testid to the Card. */}
      {/* The Link will make the entire Card clickable. */}
      <Card
        data-testid={`card-trending-${rank}`}
        className="group overflow-hidden hover-elevate active-elevate-2 cursor-pointer transition-all duration-300 relative"
      >
